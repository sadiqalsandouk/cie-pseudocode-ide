...
export function checkSyntax(code: string): SyntaxCheckResult {
  ...
  lines.forEach((line, index) => {
    ...
    // Check for invalid assignment syntax
    if (trimmedLine.includes("←")) {
      // Check for line ending with assignment operator
      if (trimmedLine.trim().endsWith("←")) {
        errors.push({
          line: lineNumber,
          message: "Assignment operator ← must be followed by a value",
          type: "error",
        });
      }

      // Check for multiple assignment operators in one line (invalid)
      const arrowCount = (trimmedLine.match(/←/g) || []).length;
      if (arrowCount > 1) {
        errors.push({
          line: lineNumber,
          message: "Multiple assignment operators (←) on one line is invalid",
          type: "error",
        });
      }

      // Check for invalid patterns like "variable ← value ←"
      if (/←\s*\w+\s*←/.test(trimmedLine)) {
        errors.push({
          line: lineNumber,
          message:
            "Invalid assignment syntax - check for extra assignment operators",
          type: "error",
        });
      }
    }
    ...
    if (!hasValidStructure) {
      errors.push({
        line: lineNumber,
        message:
          "This line does not appear to be valid pseudocode. Use Cambridge keywords and proper syntax.",
        type: "error",
      });
    }
    ...
    if (words.length > 0 && words[0].match(/^[A-Z]+$/)) {
      ...
      if (
        closingKeywords.includes(firstWord) ||
        firstWord === "NEXT" ||
        firstWord === "UNTIL"
      ) {
    ...
    if (trimmedLine.startsWith("DECLARE")) {
      if (!trimmedLine.includes(":")) {
        errors.push({
          line: lineNumber,
          message:
            "DECLARE statement must specify variable type with colon (:)",
          type: "error",
        });
      } else {
        ...
        if (colonIndex !== -1) {
          ...
          const variableNames = beforeColon
            .split(",")
            .map((name) => name.trim());
            ...
            if (varName && /^[a-zA-Z_][a-zA-Z0-9_]*$/.test(varName)) {
              ...
              if (declaredVariables.has(varNameLower)) {
                ...
                if (previousType && previousType.type !== afterColon) {
                  ...
                  errors.push({
                    line: lineNumber,
                    message: `Variable '${varName}' already declared on line ${
                      previousType?.line || "unknown"
                    }`,
                    type: "warning",
                  });
                }
                ...
                variableTypes.set(varNameLower, {
                  type: afterColon,
                  line: lineNumber,
                });
              }
    ...
    if (trimmedLine.startsWith("CONSTANT")) {
      ...
      const constantMatch = trimmedLine.match(
        /CONSTANT\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=/i
      );
      if (constantMatch) {
        ...
        variableTypes.set(constantName.toLowerCase(), {
          type: "CONSTANT",
          line: lineNumber,
        });
      }
    ...
    if (trimmedLine.startsWith("DECLARE") && trimmedLine.includes("ARRAY")) {
      if (!trimmedLine.includes("[") || !trimmedLine.includes("]")) {
        errors.push({
          line: lineNumber,
          message:
            "ARRAY declaration must include index range in square brackets [start:end]",
          type: "error",
        });
      }
    ...
    if (trimmedLine.startsWith("WHILE")) {
      if (trimmedLine.includes("(") && trimmedLine.includes(")")) {
        errors.push({
          line: lineNumber,
          message:
            "WHILE conditions should not use parentheses in Cambridge pseudocode",
          type: "error",
        });
      }
    }
      ...
      charMatches.forEach((charLiteral) => {
        ...
        if (content.length === 0) {
          errors.push({
            line: lineNumber,
            message:
              "Empty CHAR literal - CHAR must contain exactly one character",
            type: "error",
          });
          ...
          errors.push({
            line: lineNumber,
            message:
              "CHAR literal can only contain one character. Use double quotes for strings",
            type: "error",
          });
        }
      });
    ...
    if (
      trimmedLine.match(/DECLARE\s+\w+\s*:\s*CHAR/) &&
      trimmedLine.includes('"')
    ) {
      errors.push({
        line: lineNumber,
        message:
          "CHAR literals must use single quotes ('), not double quotes (\")",
        type: "error",
      });
    }
    ...
    if (
      ...
      const variableMatches = lineToCheck.match(
      ...
      if (variableMatches) {
        variableMatches.forEach((match) => {
          ...
          if (
            ["OUTPUT", "INPUT", "IF", "WHILE", "THEN"].includes(
              match.toUpperCase()
            )
          )
            return;
    ...
    if (trimmedLine.startsWith("OPENFILE")) {
      ...
      if (openfileMatch) {
        ...
        errors.push({
          line: lineNumber,
          message:
            "OPENFILE syntax should be: OPENFILE filename FOR mode (READ/write/append)",
          type: "error",
        });
      }
    }
    ...
    if (
      ...
      lines.some((line, idx) => idx > index && line.trim().startsWith("ENDFOR"))
    ) {
      const endforLine = lines.findIndex(
        (line, idx) => idx > index && line.trim().startsWith("ENDFOR")
      );
      if (endforLine !== -1) {
        errors.push({
          line: endforLine + 1,
        ...
        params.forEach((param) => {
          ...
          if (paramParts.length === 2) {
            ...
            if (paramName && /^[a-zA-Z_][a-zA-Z0-9_]*$/.test(paramName)) {
              ...
              variableTypes.set(paramName.toLowerCase(), {
                type: paramType,
                line: lineNumber,
              });
            }
  ...
  blockStack.forEach((block) => {
    errors.push({
      line: block.line,
      message: `Unmatched ${block.keyword} - missing ${
        BLOCK_KEYWORDS[block.keyword]
      }`,
      type: "error",
    });
  });
  ...
  lines.forEach((line, index) => {
    ...
    if (
      trimmedLine === "ENDIF" ||
      trimmedLine === "ENDWHILE" ||
      trimmedLine === "ENDPROCEDURE" ||
      trimmedLine === "ENDFUNCTION" ||
      trimmedLine === "ENDCASE" ||
      trimmedLine === "ENDTYPE"
    ) {
  ...
  lines.forEach((line, index) => {
    ...
    const lengthMatches = trimmedLine.match(
      /LENGTH\s*\(\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*\)/gi
    );
    if (lengthMatches) {
      lengthMatches.forEach((match) => {
        const varMatch = match.match(
          /LENGTH\s*\(\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*\)/i
        );
        if (varMatch) {
    ...
    if (trimmedLine.startsWith("FOR ") && trimmedLine.includes(" TO ")) {
      ...
      if (nextLineIndex !== -1 && nextLineIndex === index + 1) {
        errors.push({
          line: lineNumber,
          message:
            "Empty FOR loop body - consider adding statements or removing the loop",
          type: "warning",
        });
      }
    }
